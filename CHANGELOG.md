...
### Added
- **In-panel navigation for fetched pages.** Inside a URL-loaded viewer, following a link (`Enter` or click) navigates in place — relative links resolve against the page URL — with `Alt+Left`/`Alt+Right` for back/forward history. `O` opens the link under the cursor in the external browser instead. (File-backed views are unchanged: links still open externally.)
