...
- Клик по кнопке `[▶]/[▼]` → Переключить развёрнутость/свёрнутость
- Клик по title → Активировать панель (двойной клик по файловому менеджеру → выбор каталога)
- **Drag за title** → Drag-and-drop панели: ghost следует за курсором, drop-зона подсвечивается; drop на заголовок другой панели вставляет в её группу, drop между группами создаёт новую. `Escape` отменяет.
...
**Развёрнутая панель:**
- Граница с кнопками `[≡][иконка]` и заголовком (напр. `[≡][📁] Файлы`)
