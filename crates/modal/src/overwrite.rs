//! File overwrite confirmation modal dialog.
...
impl OverwriteModal {
    ...
    fn calculate_modal_width(&self, screen_width: u16) -> u16 {
        ...
        let message_width = max_line_width(&message);
        let max_option_width = OverwriteChoice::all()
            .iter()
            .map(|c| 2 + c.label().len())
            .max()
            .unwrap_or(0) as u16;
        ...
        calculate_modal_width(
            [title_width, message_width, max_option_width].into_iter(),
            screen_width,
...
impl Modal for OverwriteModal {
    ...
    fn render(&mut self, area: Rect, buf: &mut Buffer, theme: &Theme) {
        ...
        let inner = render_modal_block(modal_area, buf, "File already exists", theme);
